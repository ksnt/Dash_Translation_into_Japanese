...
5.`children`属性は特別です。規定によりその属性は常に、それを省略することができるということを意味での第一属性です。例えば`html.H1(children='Hello Dash')`は`html.H1('Hello Dash')`と同様です。また、文字列、数字、単一コンポーネント、コンポーネントのリストを含めることができます。  
